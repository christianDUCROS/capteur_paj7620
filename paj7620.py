...
#
# adaptation pour l'ESP32 /RP2  du programme Python pour le Raspberry a partir 
...
PAJ7620_ADDR_R_IDLE_TIME_0      = (PAJ7620_ADDR_BASE + 0x65)   #RW
PAJ7620_ADDR_R_IDLE_TIME_1      = (PAJ7620_ADDR_BASE + 0x66)  #RW
...
[0x65,0xB4],[0x66,0x00],[0x67,0x97],[0x68,0x01],[0x69,0xCD],[0x6A,0x01],[0x6B,0xB0],[0x6C,0x04],
...
class PAJ7620():
    ...
    def paj7620SelectBank(self, bank):
        ...
        if bank == BANK1:
            self.paj7620WriteReg(PAJ7620_REGISTER_BANK_SEL, PAJ7620_BANK1)    
    ...
    def pos_xy(self) :
        ...
        while posx ==0 and posy ==0 : 
            ax = self.paj7620ReadReg(0xAC, 1)[0]
            bx = self.paj7620ReadReg(0xAD, 1)[0]
            posx=int(bx)*256 + int(ax)
            ay = self.paj7620ReadReg(0xAE, 1)[0]
            by = self.paj7620ReadReg(0xAF, 1)[0]
            posy=int(by)*256 + int(ay)
        return(posx,posy)
